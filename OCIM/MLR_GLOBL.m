...
%func = make_MLR_function(mdl,tbl);

% get rid of outliers based on Cook's distance.
%igood = find((mdl.Diagnostics.CooksDistance)<5*mean(mdl.Diagnostics.CooksDistance));
%tbl = [DFe(igood),DIP(igood),DIN(igood),SST(igood),SI(igood),O2min(igood)];
%tbl = [DFe,DIP,DIN,SST,SI,O2min];
...
%mdl_ATL = stepwiselm(PAR_ATL,NF_ATL(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
...
%mdl_PAC = stepwiselm(PAR_PAC,NF_PAC(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
...
%mdl_IND = stepwiselm(PAR_IND,NF_IND(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
SB.IND_PAR = PAR_IND;
...
%mdl_NATL = stepwiselm(PAR_NATL,NF_NATL(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
...
%mdl_SATL = stepwiselm(PAR_SATL,NF_SATL(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
...
%mdl_NPAC = stepwiselm(PAR_NPAC,NF_NPAC(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
...
%mdl_SPAC = stepwiselm(PAR_SPAC,NF_SPAC(ido),'quadratic','Criterion','Rsquared','PRemove',0.05)
