...
Felim = nanmean(Fe_lim(:,:,1:10),3);

iposi = and(NF2d(:)>0,Felim(:)>0); % use only positive numbers.

NF  = log10(NF2d(iposi));
Felim = Felim(iposi);

NF(imag(NF)~=0) = nan;    % change imaginary number to nan.
Felim(imag(Felim)~=0) = nan;  % change imaginary number to nan.
% get rid of nan and infinity numbers.
ivalue = and(~or(isnan(NF),isnan(Felim)),~or(isinf(NF), isinf(Felim)));

NF  = NF(ivalue);
Felim = Felim(ivalue);

%NF_z  = (NF-mean(NF))/std(NF);
%Felim_z = (Felim-mean(Felim))/std(Felim);
%NF  = NF_z;
%Felim = Felim_z;
...
Felim_ATL = Felim.*ATL_srf(ivalue);

NF_PAC = NF.*PAC_srf(ivalue);
Felim_PAC = Felim.*PAC_srf(ivalue);

NF_IND = NF.*IND_srf(ivalue);
Felim_IND = Felim.*IND_srf(ivalue);

NF_ARC = NF.*ARC_srf(ivalue);
Felim_ARC = Felim.*ARC_srf(ivalue);

% ==========================================================================
fprintf('global ocean regression \n')
% linear regression 
RG_GLOB = fitlm(Felim,NF,'linear');
...
figure()
txt.Basion = 'Atlantic';
plt_conf(Felim_ATL(ido),NF_ATL(ido),txt,1)
SB.ATL_x  = Felim_ATL(ido);
SB.ATL_y = NF_ATL(ido);
...
ido = find(NF_PAC~=0);
RG_PAC = fitlm(Felim_PAC(ido),NF_PAC(ido));
...
figure()
txt.Basion = 'Pacific';
plt_conf(Felim_PAC(ido),NF_PAC(ido),txt,1)
SB.PAC_x  = Felim_PAC(ido);
SB.PAC_y  = NF_PAC(ido);
...
ido = find(NF_IND~=0);
RG_IND = fitlm(Felim_IND(ido),NF_IND(ido));
...
figure()
txt.Basion = 'Indian';
plt_conf(Felim_IND(ido),NF_IND(ido),txt,1)
SB.IND_x  = Felim_IND(ido);
SB.IND_y  = NF_IND(ido);
...
Felim_NATL = Felim.*NATL_srf(ivalue);
NF_SATL = NF.*SATL_srf(ivalue);
Felim_SATL = Felim.*SATL_srf(ivalue);

NF_NPAC = NF.*NPAC_srf(ivalue);
Felim_NPAC = Felim.*NPAC_srf(ivalue);
NF_SPAC = NF.*SPAC_srf(ivalue);
Felim_SPAC = Felim.*SPAC_srf(ivalue);

% =====================================================================
fprintf('N. Atlantic ocean regression \n')
% select data only in the N. Atlantic ocean for regression. 
ido = find(NF_NATL~=0);
RG_NATL = fitlm(Felim_NATL(ido),NF_NATL(ido));
fprintf('R^2 = %0.3f; \n',  RG_NATL.Rsquared.Adjusted)
fprintf('F statistic p-value = %5.5e \n',RG_NATL.Coefficients.pValue(2))
fprintf('Slope   = %5.2e \n',RG_NATL.Coefficients.Estimate(2))
fprintf('SlopeSE = %5.2e \n\n',RG_NATL.Coefficients.SE(2))

[p,ErrorEst] = polyfit(Felim_NATL(ido),NF_NATL(ido),1);
[NF_fit,delta] = polyval(p,Felim_NATL(ido),ErrorEst);

% quadratic regression
figure()
txt.Basion = 'N. Atlantic';
plt_conf(Felim_NATL(ido),NF_NATL(ido),txt,1)
SB.NATL_x  = Felim_NATL(ido);
...
ido = find(NF_SATL~=0);
RG_SATL = fitlm(Felim_SATL(ido),NF_SATL(ido));
...
plt_conf(Felim_SATL(ido),NF_SATL(ido),txt,1)
SB.SATL_x  = Felim_SATL(ido);
...
ido = find(NF_NPAC~=0);
RG_NPAC = fitlm(Felim_NPAC(ido),NF_NPAC(ido));
...
plt_conf(Felim_NPAC(ido),NF_NPAC(ido),txt,1)
SB.NPAC_x  = Felim_NPAC(ido);
...
ido = find(NF_SPAC~=0);
RG_SPAC = fitlm(Felim_SPAC(ido),NF_SPAC(ido));
...
plt_conf(Felim_SPAC(ido),NF_SPAC(ido),txt,1)
SB.SPAC_x  = Felim_SPAC(ido);
